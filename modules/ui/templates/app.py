...
def LoadSession(sessionName: str):
    ...
    try:
        ...
        processor = AProcessor(name="frasier", modelID=config.modelID, promptName=config.prompt, llmPool=llmPool, promptsManager=promptsManager, services=clientPool, messenger=messenger, outputCB=logger.Receiver, collection=sessionName)
        ...
        p = os.path.join(sessionPath, "frasier_history.json")
        if os.path.exists(p):
            with open(p, "r") as f:
...
def main():
    ...
    #parser.add_argument('--share',type=bool,default=False, help="Whether to create a publicly shareable link for frasier.")
...
def generate_response(message):
    try:
        ...
        while True:
            ...
            if ">" == channel:
                threadLLM.join()
                with open(os.path.join(config.chatHistoryPath, sessionName, "frasier_history.json"), "w") as f:
...
def new_chat():
    global sessionName
    with lock:
        sessionName = "frasier_" + str(int(time.time()))
...
def load_history():
    ...
    with lock:
        ...
        if needLoading:
            LoadSession(sessionName=sessionName)
        historyPath = os.path.join(config.chatHistoryPath, sessionName, "frasier_history.json")
...
def list_histories():
    with lock:
        ...
        for d in os.listdir(config.chatHistoryPath):
            p = os.path.join(config.chatHistoryPath, d, "frasier_history.json")
            if os.path.exists(p):
                with open(p, "r") as f:
                    try:
                        content = json.load(f)
        ...
        return jsonify(sorted(histories, key=lambda x: os.path.getmtime(os.path.join(config.chatHistoryPath, x[0], "frasier_history.json")), reverse = True))
